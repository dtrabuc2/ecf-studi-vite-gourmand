                ...
                <section class="card border-0 shadow-sm">
                    <div class="card-body p-4 p-md-5">
                        <div class="text-center mb-4">
                            ...
                            <p class="mb-0">Votre demande #<?= $order->getId() ?> a été créée pour le <?= $escape($order->getDeliveryDate()) ?>.</p>
                        </div>
                        ...
                        <dl class="row text-start mt-4">
                            ...
                            <dd class="col-sm-7">Espèces sur place</dd><?php if ($order->getCustomization()): ?><dt class="col-sm-5">Options choisies</dt><dd class="col-sm-7"><?= $escape($order->getCustomization()) ?></dd><?php endif; ?><?php if ($order->getDeliveryInstructions()): ?><dt class="col-sm-5">Instructions</dt><dd class="col-sm-7"><?= $escape($order->getDeliveryInstructions()) ?></dd><?php endif; ?><dt class="col-sm-5">Nombre de personnes</dt><dd class="col-sm-7"><?= $order->getNumberOfPeople() ?></dd><dt class="col-sm-5">Prix menu après remise</dt><dd class="col-sm-7"><?= number_format($order->getMenuPrice(), 2, ',', ' ') ?> €</dd><dt class="col-sm-5">Remise</dt><dd class="col-sm-7"><?= number_format($order->getDiscountRate(), 0) ?> %</dd><dt class="col-sm-5">Livraison</dt><dd class="col-sm-7"><?= number_format($order->getDeliveryCost(), 2, ',', ' ') ?> €</dd><dt class="col-sm-5">Total</dt><dd class="col-sm-7"><?= number_format($order->getTotalPrice(), 2, ',', ' ') ?> €</dd></dl><a class="btn btn-primary" href="/orders">Voir mes commandes</a></div></section></div></div></div></main>
